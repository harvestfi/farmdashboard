...
export class VaultStatsComponent implements OnInit {
  ...
  loadSingleVaultProfit(): void {
    this.hardWorkService.getHardWorkHistoryDataByAddress(
      this.vaultAddress,
      StaticValues.NETWORKS.get(this.vaultNetwork),
    )
      .pipe(
        filter(data => !!data.length),
        takeUntil(this.destroy$),
  ...
  loadSingleVaultAPY(): void {
    this.rewardsService.getHistoryRewards(
      this.vaultAddress,
      StaticValues.NETWORKS.get(this.vaultNetwork),
    )
      .pipe(
  ...
  loadSingleVaultUsers(): void {
    this.tvlService.getHistoryTvlByVault(
      this.vaultAddress,
      StaticValues.NETWORKS.get(this.vaultNetwork),
    )
      .pipe(
