...
export class TheGraphService {
  ...
  getGraphAnalytics(id): any {
      const subGraphsQuery = `{"operationName":"subgraphDaily","variables":{` +
          ...
          `"query":"query subgraphDaily($id: ID!, $orderBy: SubgraphDeploymentDailyData_orderBy, $orderDirection: OrderDirection)` +
          `@analytics {  subgraphDeploymentDailyDatas(where: {subgraphDeployment: $id},` +
          `orderBy: $orderBy, orderDirection: $orderDirection,` +
          `first: 1000) {   id    dayNumber    dayStart    dayEnd    queryFeesAmount    signalledTokens    __typename  }}"}`;
