...
export class HardWorkHistoryDialogComponent implements AfterViewInit {
  ...
  private loadData(): void {
    this.httpService.getHardWorkHistoryData().subscribe(data => {
      ...
      const hw = new Map<number, number>();
      let savedGas = 0;
      data?.forEach(dto => {
        savedGas += dto.savedGasFeesSum;
        hw.set(dto.blockDate, savedGas);
      });
      chartBuilder.initVariables(1);
      hw.forEach((fees, date) => chartBuilder.addInData(0, date, fees / 1000));
      this.handleData(chartBuilder, [
        ['Saved Gas Fees K$', 'right', '#0085ff']
      ]);
    });
  }
